...
void trxRfSpiInterfaceInit(uint8 prescalerValue)
{
	...
	/* SPI is configured in USCI_B0	 */

	/* Keep peripheral in reset state*/
	UCB0CTL1 |= UCSWRST;			// UCSWRST (0x01) USCI Software Reset (1 Enabled. USCI logic held in reset state)
	...
	*/
	...
	UCB0CTL1 |=  UCSSEL_2;							// USCI clock source select. SCLK do SPI trabalha com SMCLK
	UCB0BR1   =  0x00;
	...
	*/
	// P2SEL       |= BIT1 (MOSI)		 + BIT2 (MISO)		  + BIT3 (SCLK)
	TRXEM_PORT_SEL |= TRXEM_SPI_MOSI_PIN + TRXEM_SPI_MISO_PIN + TRXEM_SPI_SCLK_PIN;

	// P2SEL       |= BIT0 (CSn)
	TRXEM_PORT_SEL &= ~TRXEM_SPI_SC_N_PIN;	// P2.0 (CSn)
