...
/************************************************************************/
/*  - name: config_MSP430_ONE_WIRE
 *  - Description: configures neccesary ports and clock for one-wire communication
 *  - input data: --
 *  - output data: --
 ***********************************************************************/
...
void config_DS2784(void){
	...
	OWWriteByte(0x00);					// register address
	OWWriteByte(0x03);					// value to be written

	reset= OneWireReset();              // PROTECTOR THRESHOLD REGISTER
	OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
	OWWriteByte(0x6C);					// write operation
	OWWriteByte(0x7F);					// register address
	OWWriteByte(0xF0);					// value to be written

	reset= OneWireReset();				// STATUS REGISTER
	OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
	OWWriteByte(0x6C);					// write operation
	OWWriteByte(0x01);					// register address
	...
	OWWriteByte(0x22);					// value to be written

	reset= OneWireReset();				// ACCUMULATED CURRENT - LSB REGISTER
	OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
	OWWriteByte(0x6C);					// write operation
	OWWriteByte(0x11);					// register address
	OWWriteByte(0x60);					// value to be written
...
void measurement_data_DS2784(void){
   ...
   // TEMPERATURE MEASUREMENT
    ...
    OWWriteByte(0x0B);					// register address
    aux=OWReadByte();
    tr_lsb=aux>>5;
    ...
    OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
    OWWriteByte(0x69);					// read operation
    OWWriteByte(0x09);					// register address
    avc_lsb=OWReadByte();

    reset= OneWireReset();              // AVERAGE CURRENT MEASUREMENT - MSB REGISTER
    OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
    OWWriteByte(0x69);					// read operation
    OWWriteByte(0x08);					// register address
    avc_msb=OWReadByte();
    ...
    OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
	OWWriteByte(0x6C);					// write operation
	OWWriteByte(0x79);					// register address
	OWWriteByte(0x00);					// value to be written

	reset= OneWireReset();              // MSB CURRENT GAIN REGISTER
	OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
    OWWriteByte(0x6C);					// write operation
    OWWriteByte(0x78);					// register address
   ...
   //CURRENT MEASUREMENT
    ...
    OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
    OWWriteByte(0x69);					// read operation
    OWWriteByte(0x0F);					// register address
    cr_lsb=OWReadByte();
    ...
    OWWriteByte(0xCC);					// eeprom address (only one slave on bus, CC is used)
    OWWriteByte(0x69);					// read operation
    OWWriteByte(0x0D);					// register address
    aux=OWReadByte();
    vr_lsb=aux>>5;
