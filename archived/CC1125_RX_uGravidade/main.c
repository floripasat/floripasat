/*
                          ...
                          |                               |
                          |                               |
ENE_3V3_PA_TTC <----------+ P1.1                     P5.4 +(32)----------> LED_SYSTEM
                          |                               |
   OBDH_STATUS <----------> P1.2                          |
                          |                               |
      XIN_OBDH +----------+ XIN                      	  |
                          |                (USCI_A2) P9.2 +(70)----------> UART0_OBDH_TX (UART)
     XOUT_OBDH +----------+ XOUT           (USCI_A2) P9.3 +(71)----------> UART0_OBDH_RX (UART)
                          |                               |
...
int main(void) {
    ...
    __enable_interrupt();
     ...
     //runTX();                                         //configurator Radio Tx
    runRX();                                            //configurator Radio Rx
}
/*
...
static void initMCU(void) {
    ...
    // Init clocks and I/O					// -----> Verificar essa fun��o para utili configurar o MSP <-------
...
static void runTX(void) {
	...
	while(1){
		...
		if(1) {										// --------------->  Definir condi��o de entrada <----------------
		   ...
		   // } while (1);							// --------------->  Definir condi��o de saida <----------------
		}
