...
#define one_wire_pin_number 5

#define current_sensor_gain 133.37
#define current_sensor_resistance 0.06213
#define bat_monitor_current_sensor_resistance 0.025
...
volatile float current[40] = {0};
volatile float voltage[40] = {0};
volatile float volt = 0;
volatile float curr = 0;
volatile float previous_power = 0;
volatile float power[40] = {0};
...
void config_MSP430(void){
	...
	ADC12CTL0 = ADC12ON+MSC+SHT0_5 + REFON + REF2_5V;  // Turn on ADC12, set sampling time, 16 ADC12CLK cycles, Vref=2.5V
...
void measurement_adc(void){                             // ADC  function
...
void outp(int bit){
	...
	DIR_P_1Wire |= one_wire_pin;			//sets P1.0 as output

	if(bit == 0){
		P_1WireOUT &= ~one_wire_pin;           //drives P1.0 low
	}
	else{
		DIR_P_1Wire &= ~one_wire_pin;             //releases the bus
	}
}
/************************************************************************/
/*  - name: inp
...
int inp(void){
	...
	DIR_P_1Wire &= ~one_wire_pin;          //sets P1.0 as input
	result= (P_1WireIN & one_wire_pin);         //prepares the bit on P1.0 to be returned

	return (result >> one_wire_pin_number);		        //returns the bit on P1.0
...
void task_scheduler(void){
	...
	e=e<<8;
	d=d+e;
	curr = 2*(d*0.00061035)/(current_sensor_gain*current_sensor_resistance);
